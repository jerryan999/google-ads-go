// Copyright 2024 Google LLC
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.34.2
// 	protoc        v5.27.1
// source: google/ads/googleads/v17/errors/video_campaign_error.proto

package errors

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// Enum describing possible video campaign errors.
type VideoCampaignErrorEnum_VideoCampaignError int32

const (
	// Enum unspecified.
	VideoCampaignErrorEnum_UNSPECIFIED VideoCampaignErrorEnum_VideoCampaignError = 0
	// The received error code is not known in this version.
	VideoCampaignErrorEnum_UNKNOWN VideoCampaignErrorEnum_VideoCampaignError = 1
	// Cannot modify the video campaign without reservation.
	// See https://support.google.com/google-ads/answer/9547606.
	VideoCampaignErrorEnum_MUTATE_REQUIRES_RESERVATION VideoCampaignErrorEnum_VideoCampaignError = 2
)

// Enum value maps for VideoCampaignErrorEnum_VideoCampaignError.
var (
	VideoCampaignErrorEnum_VideoCampaignError_name = map[int32]string{
		0: "UNSPECIFIED",
		1: "UNKNOWN",
		2: "MUTATE_REQUIRES_RESERVATION",
	}
	VideoCampaignErrorEnum_VideoCampaignError_value = map[string]int32{
		"UNSPECIFIED":                 0,
		"UNKNOWN":                     1,
		"MUTATE_REQUIRES_RESERVATION": 2,
	}
)

func (x VideoCampaignErrorEnum_VideoCampaignError) Enum() *VideoCampaignErrorEnum_VideoCampaignError {
	p := new(VideoCampaignErrorEnum_VideoCampaignError)
	*p = x
	return p
}

func (x VideoCampaignErrorEnum_VideoCampaignError) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (VideoCampaignErrorEnum_VideoCampaignError) Descriptor() protoreflect.EnumDescriptor {
	return file_errors_video_campaign_error_proto_enumTypes[0].Descriptor()
}

func (VideoCampaignErrorEnum_VideoCampaignError) Type() protoreflect.EnumType {
	return &file_errors_video_campaign_error_proto_enumTypes[0]
}

func (x VideoCampaignErrorEnum_VideoCampaignError) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use VideoCampaignErrorEnum_VideoCampaignError.Descriptor instead.
func (VideoCampaignErrorEnum_VideoCampaignError) EnumDescriptor() ([]byte, []int) {
	return file_errors_video_campaign_error_proto_rawDescGZIP(), []int{0, 0}
}

// Container for enum describing possible video campaign errors.
type VideoCampaignErrorEnum struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields
}

func (x *VideoCampaignErrorEnum) Reset() {
	*x = VideoCampaignErrorEnum{}
	if protoimpl.UnsafeEnabled {
		mi := &file_errors_video_campaign_error_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *VideoCampaignErrorEnum) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*VideoCampaignErrorEnum) ProtoMessage() {}

func (x *VideoCampaignErrorEnum) ProtoReflect() protoreflect.Message {
	mi := &file_errors_video_campaign_error_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use VideoCampaignErrorEnum.ProtoReflect.Descriptor instead.
func (*VideoCampaignErrorEnum) Descriptor() ([]byte, []int) {
	return file_errors_video_campaign_error_proto_rawDescGZIP(), []int{0}
}

var File_errors_video_campaign_error_proto protoreflect.FileDescriptor

var file_errors_video_campaign_error_proto_rawDesc = []byte{
	,
}

var (
	file_errors_video_campaign_error_proto_rawDescOnce sync.Once
	file_errors_video_campaign_error_proto_rawDescData = file_errors_video_campaign_error_proto_rawDesc
)

func file_errors_video_campaign_error_proto_rawDescGZIP() []byte {
	file_errors_video_campaign_error_proto_rawDescOnce.Do(func() {
		file_errors_video_campaign_error_proto_rawDescData = protoimpl.X.CompressGZIP(file_errors_video_campaign_error_proto_rawDescData)
	})
	return file_errors_video_campaign_error_proto_rawDescData
}

var file_errors_video_campaign_error_proto_enumTypes = make([]protoimpl.EnumInfo, 1)
var file_errors_video_campaign_error_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_errors_video_campaign_error_proto_goTypes = []any{
	(VideoCampaignErrorEnum_VideoCampaignError)(0), // 0: google.ads.googleads.v17.errors.VideoCampaignErrorEnum.VideoCampaignError
	(*VideoCampaignErrorEnum)(nil),                 // 1: google.ads.googleads.v17.errors.VideoCampaignErrorEnum
}
var file_errors_video_campaign_error_proto_depIdxs = []int32{
	0, // [0:0] is the sub-list for method output_type
	0, // [0:0] is the sub-list for method input_type
	0, // [0:0] is the sub-list for extension type_name
	0, // [0:0] is the sub-list for extension extendee
	0, // [0:0] is the sub-list for field type_name
}

func init() { file_errors_video_campaign_error_proto_init() }
func file_errors_video_campaign_error_proto_init() {
	if File_errors_video_campaign_error_proto != nil {
		return
	}
	if !protoimpl.UnsafeEnabled {
		file_errors_video_campaign_error_proto_msgTypes[0].Exporter = func(v any, i int) any {
			switch v := v.(*VideoCampaignErrorEnum); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_errors_video_campaign_error_proto_rawDesc,
			NumEnums:      1,
			NumMessages:   1,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_errors_video_campaign_error_proto_goTypes,
		DependencyIndexes: file_errors_video_campaign_error_proto_depIdxs,
		EnumInfos:         file_errors_video_campaign_error_proto_enumTypes,
		MessageInfos:      file_errors_video_campaign_error_proto_msgTypes,
	}.Build()
	File_errors_video_campaign_error_proto = out.File
	file_errors_video_campaign_error_proto_rawDesc = nil
	file_errors_video_campaign_error_proto_goTypes = nil
	file_errors_video_campaign_error_proto_depIdxs = nil
}
